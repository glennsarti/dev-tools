...
cmake -G `"MinGW Makefiles`" -DBOOST_ROOT=`"$($boostDir.FullName)`" -DYAMLCPP_ROOT=`"$($yamlcppDir.FullName)`" -DBOOST_STATIC=ON -DCURL_STATIC=ON -DCMAKE_INSTALL_PREFIX=`"C:\Program Files\FACTER`" -DCMAKE_PREFIX_PATH=`"$($leathermanDir.FullName);$($curlDir.FullName);$($hoconDir.FullName)`" .
...
$($INSTALL_LOCATION)\build-facter.bat     Builds facter

$($INSTALL_LOCATION)\test-facter.bat      Runs tests
